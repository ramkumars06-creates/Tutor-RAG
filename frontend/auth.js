...
async function fetchQuotaStatus() {
  try {
    ...
    const res = await fetch(`${BACKEND_URL}/api/rate-status`, {
    ...
    console.warn('Could not fetch quota status:', e.message);
  }
}
...
function updateQuotaUI(used, limit) {
  ...
  document.getElementById('quotaBarText').textContent = `${used} / ${limit} used`;
  document.getElementById('quotaBarFill').style.width = pct + '%';
  document.getElementById('quotaBarFill').style.background =
    pct >= 90 ? 'linear-gradient(90deg,#f43f5e,#fb7185)' :
    pct >= 70 ? 'linear-gradient(90deg,#fbbf24,#f59e0b)' :
    'linear-gradient(90deg,#7c6aff,#00d4ff)';
  // Update header quota badge color
  const badge = document.getElementById('quotaBadge');
  if (badge) {
    badge.style.background = pct >= 90 ? 'rgba(244,63,94,0.2)' : 'rgba(124,106,255,0.15)';
    badge.style.borderColor = pct >= 90 ? 'rgba(244,63,94,0.4)' : 'rgba(124,106,255,0.3)';
  }
}
